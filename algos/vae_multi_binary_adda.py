...
from algos.network_structure import FCDecoder, CnnBinaryDiscriminator
from algos.vae_binary import BinaryVAE
import algos.tool_func as tool

class MultiCategoryADDA(BinaryVAE):
    def __init__(self, obj_encoder_type = 'CNNLatentDistrib', latent_length=3, last_conv_channel=1, discriminator_conv_size=16, lr=0.0002, device="cuda:0"):
        ...
        super(MultiCategoryADDA, self).__init__(obj_encoder_type=obj_encoder_type, latent_length=latent_length, last_conv_channel=last_conv_channel, lr=lr, device=device)
        self.src_label = 0.9
        self.obj_label = 0.1
    ...
    def load_models(self, outf, method):
        tool.load_models(outf, method, src_encoder=self.net_encoder_sourse, obj_encoder=self.net_encoder_target, decoder=self.net_decoder, 
