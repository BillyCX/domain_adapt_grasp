...
class MultiCategoryVAE(BinaryVAE):
    def __init__(self, obj_encoder_type = 'CNNLatentDistrib', latent_length=3, last_conv_channel=1, discriminator_conv_size=16, lr=0.0002 ):
        ...
        self.obj_mean = collections.deque(maxlen=1000)
        self.obj_mean.append(0)
        self.pre_updated_mean = 0
    ...
    def load_models(self, outf, method):
        tool.load_models(outf, method, src_encoder=self.net_encoder_sourse, decoder=self.net_decoder, 
                                        discriminator_noobj=None, discriminator=self.net_discriminator)
    ...
    def get_discriminator_loss(self, src_conv, obj_conv, noobj_conv, dis_obj_src):   
        ###########################
        # train to recognize src
        ###########################
        errD_src, _ = self.get_loss_on_label(src_conv, self.label_src, self.net_discriminator_noobj, detach=True)

        ###########################
        # train to recognize obj
        # ########################### 
        label_obj_tuned = torch.where(dis_obj_src < np.mean(self.obj_mean), self.label_noobj, self.label_src)
        # print(dis_obj_src)
        # print(label_obj_tuned)
        # print(np.mean(self.obj_mean))
        errD2_obj, _ = self.get_loss_on_label(obj_conv, label_obj_tuned, self.net_discriminator, detach=True) 

        ###########################
        # train to recognize noobj
        ###########################  
        errD2_noobj, _ = self.get_loss_on_label(noobj_conv, self.label_noobj, self.net_discriminator_noobj, detach=True)

        # print(dis_obj)
        # print(dis_obj2)
        # errD = errD_src + errD_obj + errD_noobj
        return errD_src, errD2_obj, errD2_noobj

    def update_discriminator(self, errD_src, errD2_obj, errD2_noobj):
        err1 = errD_src + errD2_noobj
        err2 = errD2_obj
    ...
    def update_generator(self, errG_obj, src_autoed_loss):
        ...
        autoed_loss = errG_obj*0.01 + src_autoed_loss*1
    ...
    def update_net(self, data_src, data_obj, data_noobj, epoch, iteration, outf, method):
        ...
        if self.label_src is None:
            ...
            self.label_noobj = torch.full((batch_size,), self.noobj_label, device=self.device)
        ...
        _, dis_obj_src = self.get_loss_on_label(obj_conv, self.label_src, self.net_discriminator_noobj)
        ############################
        # (2) Update G network: maximize log(D(G(z)))
        ###########################
        errG_obj, _ = self.get_loss_on_label(obj_conv, self.label_src, self.net_discriminator)
        ...
        ###########################
        # train with real
        errD_src, errD2_obj, errD2_noobj  = self.get_discriminator_loss(src_conv, obj_conv, noobj_conv, dis_obj_src)
        ...
        print('[%d][%d] Loss: %.4f %.4f %.4f | %.4f %.4f' % 
            (epoch, iteration, errD_src.item(), errD2_obj.item(), errD2_noobj.item(), errG_obj.item(), src_autoed_loss.item()))   
        ...
        if iteration == 0:
            ...
            tool.save_models(outf, method, src_encoder = self.net_encoder_sourse, obj_encoder = self.net_encoder_target, decoder = self.net_decoder, 
                                                discriminator = self.net_discriminator, discriminator_noobj = None)
